...
class main_window:
    ...
    def show_cal(self, month, year):
        ...
        go_to_frame = self.display_go_to_frame(calendar_canvas)
        today = Button(calendar_info_frame, text="Today", command=lambda: self.show_cal(current_month, current_year),bg="white",font=font, fg="black")
        ...
        today.grid(row=0, column=3)
        calendar_canvas.grid(row=0, column=1, sticky="N")
        go_to_frame.grid(row=4, column=0)
        self.root.columnconfigure(1, weight=1)
        calendar_canvas.columnconfigure(0, weight=1)

    def display_go_to_frame(self, parent):
        frame = Frame(parent, bg="white")
        valid_input = self.root.register(self.only_numbers)
        explanation_label = Label(frame, text="Go to date")
        year_entry_var = StringVar()
        year_entry = Entry(frame, textvariable=year_entry_var, validate="key", validatecommand=(valid_input,"%P",4,False))
        month_var = StringVar()
        month_var.set("Jan")
        month_options = OptionMenu(frame,month_var, *calendar.month_abbr[1:13])
        go_btn = Button(frame, text="Go", command=lambda: self.go_to_date(year_entry_var, month_var))
        explanation_label.pack(side=LEFT)
        year_entry.pack(side=LEFT)
        month_options.pack(side=LEFT)
        go_btn.pack(side=LEFT)
        return frame

    def go_to_date(self, year_entry, month):
        month = datetime.datetime.strptime(month.get(), "%b").month
        if year_entry.get() == "":
            current_year = datetime.datetime.now().year
            year_entry.set(current_year)
            year_entry = current_year
        elif int(year_entry.get()) >= 1952 and int(year_entry.get()) <= 2040:
            year_entry = int(year_entry.get())

        self.show_cal(month, year_entry)
    ...
    def show_left_navigation(self):
        left_nav_frame = Frame(self.root, background="#373d39", height=460, relief='raised', borderwidth=2)
        hs = self.root.winfo_screenheight()
        change_pass_btn = Button(left_nav_frame, text="Change password", border=0, bg="#373d39", fg="white")
        log_out = Button(left_nav_frame, text="Log out", border=0, bg="#373d39", fg="white")
        ...
        my_profile_btn = Button(left_nav_frame, font=("Arial",10,"bold"), border=0, bg="#373d39", fg="white", command=lambda but1=[change_pass_btn,log_out]: self.trigger_profile_subbuttons(but1))
        ...
        log_out.grid_forget()
        left_nav_frame.grid(row=0, sticky="nsew", ipadx=70)
        details_btn = Button(left_nav_frame, text="Details", border=0, bg="#373d39", fg="white",font=("Arial", 10, "bold"), command=self.user_transaction_plot_window)
        converter_btn = Button(left_nav_frame, text="Converter", border=0, bg="#373d39", fg="white",font=("Arial",10,"bold"), command=self.converter_window)
    ...
    def converter_window(self):
        ...
        from_entry = Entry(converter_frame, textvariable=from_entry_var, validate="key",validatecommand=(valid_input, '%P', 15, True))
    ...
    def only_numbers(self, afterInput, maxlength, allow_dots):
        import re
        ...
        if allow_dots:
            ...
            result = regex.match(afterInput)
            return (afterInput == ""
                    or (len(afterInput) <= int(maxlength)
                        and result is not None
                        and result.group(0) != ""))
