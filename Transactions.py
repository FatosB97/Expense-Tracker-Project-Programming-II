...
class Transactions:
    ...
    def checkfor_new_categories(self, options_menu):
    ...
    def display_transaction_section(self):
        ...
        transactions_edit = self.past_transactions_details_edit(past_transactions_frame)
        ...
        # btn command change
        income_btn["command"] = lambda fired_from=income_btn: self.get_date_clicked(fired_from)
        expense_btn["command"] = lambda fired_from=expense_btn: self.get_date_clicked(fired_from)

        # progressBar configure
        percentage_ofincome_expense = self.get_income_expense(self.user)
        expense_percentage["value"] = percentage_ofincome_expense[1]
        income_percentage["value"] = percentage_ofincome_expense[0]
    ...
    def restrict_maxlength(self, val, maxlength, *args):
        ...
        if len(value) > maxlength:
            print(f"paaaaasssseeeed {len(value)}")
    ...
    def change_transactions_details(self, transaction_array, past_transactions_widget):
        ...
        labels_to_edit = [label for label in children if "customlabel" in str(label) or "combobox" in str(label)]  # or "combobox" in str(label)]
        i=0
        j=0
    ...
    # this shows all the transactions on the day selected and also allows edit
    def past_transactions_details_edit(self, parent):
        font = ("Calibri", 12)
        font2 = ("Calibri", 11)
        ...
        if fired_from_btn["text"] == "Edit" or all(widget.tag.get() != "" for widget in widgets):
            ...
            for label_widget in widgets:
                if fired_from_btn["text"] == "Edit":
                    ...
                    label_widget.tag.grid(row=rowP, column=columnP,sticky="W")
                    ...
                    if label_widget.tag.get() != "":
                        ...
                        label_widget.grid(row=rowP, column=columnP,sticky="W")
    ...
    def get_income_expense(self, username):
        expense_amount = 0
        income_amount = 0
        ...
        for inc, exp in itertools.zip_longest(incomes, expenses, fillvalue=0):
            income_amount += int(inc)
            expense_amount += int(exp)
        day_balance = expense_amount + income_amount
        income_percentage = 0
        expense_percentage = 0
        if day_balance != 0:
            income_percentage = income_amount / day_balance * 100
            expense_percentage = expense_amount / day_balance * 100

        return round(income_percentage, 2), round(expense_percentage, 2)

    def get_date_clicked(self, fired_from):
